...
{
	return fl_equal(self.x, other.x) &&
		   fl_equal(self.y, other.y);
}
...
void vm_vec_projection_onto_plane(vec3d *projection, const vec3d *src,
								  const vec3d *unit_normal)
{
	float mag;

	Assert((vm_vec_mag(unit_normal) > 0.999f) &&
		   (vm_vec_mag(unit_normal) < 1.001f));
...
{
	float D;    // plane constant in Ax+By+Cz+D = 0
				//   or   dot(X,n) - dot(Xp,n) = 0, so D = -dot(Xp,n)
	float dist;

	Assert((vm_vec_mag(plane_normal) > 0.999f) &&
		   (vm_vec_mag(plane_normal) < 1.001f));
...
{
	dest->xyz.x = (src0->xyz.x + src1->xyz.x +
				   src2->xyz.x + src3->xyz.x) * 0.25f;
	dest->xyz.y = (src0->xyz.y + src1->xyz.y +
				   src2->xyz.y + src3->xyz.y) * 0.25f;
	dest->xyz.z = (src0->xyz.z + src1->xyz.z +
				   src2->xyz.z + src3->xyz.z) * 0.25f;
	return dest;
}
...
// useful if you want to compare distances
float vm_vec_mag_squared(const vec3d *v)
{
	return (v->xyz.x * v->xyz.x) +
		   (v->xyz.y * v->xyz.y) +
		   (v->xyz.z * v->xyz.z);
}
...
{
#if _M_IX86_FP < 1
	return 1.0f / sqrt((v->xyz.x * v->xyz.x) +
					   (v->xyz.y * v->xyz.y) +
					   (v->xyz.z * v->xyz.z));
#else
	float x = (v->xyz.x * v->xyz.x) +
			  (v->xyz.y * v->xyz.y) +
			  (v->xyz.z * v->xyz.z);
...
{
	if ((fl_abs(src0->xyz.x - src1->xyz.x) < 1e-4) &&
		(fl_abs(src0->xyz.y - src1->xyz.y) < 1e-4) &&
...
static matrix *sincos_2_matrix(matrix *m, float sinp, float cosp,
							   float sinb, float cosb, float sinh, float cosh)
{
...
// generate the vectors for the vm_vector_2_matrix() and
// vm_vector_2_matrix_norm() functions so we can avoid goto
void vm_vector_2_matrix_gen_vectors(matrix *m)
{
	...
	if ((zvec->xyz.x == 0.0f) && (zvec->xyz.z == 0.0f)) {
		// forward vec is straight up or down
		m->vec.rvec.xyz.x = 1.0f;
		m->vec.uvec.xyz.z = (zvec->xyz.y < 0.0f) ? 1.0f : -1.0f;

		m->vec.rvec.xyz.y = m->vec.rvec.xyz.z = 0.0f;
		m->vec.uvec.xyz.x = m->vec.uvec.xyz.y = 0.0f;
	} else {
						   ...
						   const vec3d *rvec)
{
	...
	if (uvec == NULL) {
		if (rvec == NULL) {
			...
			// now recompute right vector,
			// in case it wasn't entirely perpendiclar
...
matrix *vm_vector_2_matrix_norm(matrix *m, const vec3d *fvec,
								const vec3d *uvec, const vec3d *rvec)
{
	vec3d *xvec = &m->vec.rvec;
	vec3d *yvec = &m->vec.uvec;
	vec3d *zvec = &m->vec.fvec;
...
//
// Goober5000: FYI, the result of rotating a normalized vector through a
// rotation matrix will also be a normalized vector.  It took me awhile to
// verify online that this was true. ;)
vec3d *vm_vec_rotate(vec3d *dest, const vec3d *src, const matrix *m)
{
	Assert(dest != src);

	dest->xyz.x = (src->xyz.x * m->vec.rvec.xyz.x) +
				  (src->xyz.y * m->vec.rvec.xyz.y) +
				  (src->xyz.z * m->vec.rvec.xyz.z);

	dest->xyz.y = (src->xyz.x * m->vec.uvec.xyz.x) +
				  (src->xyz.y * m->vec.uvec.xyz.y) +
				  (src->xyz.z * m->vec.uvec.xyz.z);

	dest->xyz.z = (src->xyz.x * m->vec.fvec.xyz.x) +
				  (src->xyz.y * m->vec.fvec.xyz.y) +
				  (src->xyz.z * m->vec.fvec.xyz.z);
...
vec3d *vm_vec_unrotate(vec3d *dest, const vec3d *src, const matrix *m)
{
	...
	dest->xyz.x = (src->xyz.x * m->vec.rvec.xyz.x) +
				  (src->xyz.y * m->vec.uvec.xyz.x) +
				  (src->xyz.z * m->vec.fvec.xyz.x);

	dest->xyz.y = (src->xyz.x * m->vec.rvec.xyz.y) +
				  (src->xyz.y * m->vec.uvec.xyz.y) +
				  (src->xyz.z * m->vec.fvec.xyz.y);

	dest->xyz.z = (src->xyz.x * m->vec.rvec.xyz.z) +
				  (src->xyz.y * m->vec.uvec.xyz.z) +
				  (src->xyz.z * m->vec.fvec.xyz.z);
...
matrix *vm_matrix_x_matrix(matrix *dest, const matrix *src0, const matrix *src1)
{
	...
	dest->vec.rvec.xyz.x = vm_vec_dot3(src0->vec.rvec.xyz.x,
									   src0->vec.uvec.xyz.x,
									   src0->vec.fvec.xyz.x,
									   &src1->vec.rvec);
	dest->vec.uvec.xyz.x = vm_vec_dot3(src0->vec.rvec.xyz.x,
									   src0->vec.uvec.xyz.x,
									   src0->vec.fvec.xyz.x,
									   &src1->vec.uvec);
	dest->vec.fvec.xyz.x = vm_vec_dot3(src0->vec.rvec.xyz.x,
									   src0->vec.uvec.xyz.x,
									   src0->vec.fvec.xyz.x,
									   &src1->vec.fvec);

	dest->vec.rvec.xyz.y = vm_vec_dot3(src0->vec.rvec.xyz.y,
									   src0->vec.uvec.xyz.y,
									   src0->vec.fvec.xyz.y,
									   &src1->vec.rvec);
	dest->vec.uvec.xyz.y = vm_vec_dot3(src0->vec.rvec.xyz.y,
									   src0->vec.uvec.xyz.y,
									   src0->vec.fvec.xyz.y,
									   &src1->vec.uvec);
	dest->vec.fvec.xyz.y = vm_vec_dot3(src0->vec.rvec.xyz.y,
									   src0->vec.uvec.xyz.y,
									   src0->vec.fvec.xyz.y,
									   &src1->vec.fvec);

	dest->vec.rvec.xyz.z = vm_vec_dot3(src0->vec.rvec.xyz.z,
									   src0->vec.uvec.xyz.z,
									   src0->vec.fvec.xyz.z,
									   &src1->vec.rvec);
	dest->vec.uvec.xyz.z = vm_vec_dot3(src0->vec.rvec.xyz.z,
									   src0->vec.uvec.xyz.z,
									   src0->vec.fvec.xyz.z,
									   &src1->vec.uvec);
	dest->vec.fvec.xyz.z = vm_vec_dot3(src0->vec.rvec.xyz.z,
									   src0->vec.uvec.xyz.z,
									   src0->vec.fvec.xyz.z,
									   &src1->vec.fvec);
...
// Compute the outer product of A = A * transpose(A).
// 1x3 vector becomes 3x3 matrix.
...
// 0.0f means *nearest_point is *p0, 1.0f means it's *p1. 2.0f means it's
// beyond p1 by 2x.
								 ...
								 const vec3d *p1, const vec3d *int_pnt)
{
	...
	// Normalize vector so we don't have to divide by dot product.
	mag = vm_vec_normalize(&norm);
...
void vm_orthogonalize_matrix(matrix *m_src)
{
	...
	if (umag <= 0.0f) {  // no up vector to use..
		if (rmag <= 0.0f) {  // no right vector either, so make something up
			if (!m->vec.fvec.xyz.x && !m->vec.fvec.xyz.z && m->vec.fvec.xyz.y) {
				 // vertical vector
...
void vm_fix_matrix(matrix *m)
{
	...
	if (fmag <= 0.0f) {
		if ((umag > 0.0f) && (rmag > 0.0f) &&
			!vm_test_parallel(&m->vec.uvec, &m->vec.rvec)) {
	...
	if ((umag <= 0.0f) || vm_test_parallel(&m->vec.fvec, &m->vec.uvec)) {
		// no up vector to use..
		if ((rmag <= 0.0f) || vm_test_parallel(&m->vec.fvec, &m->vec.rvec)) {
			// no right vector either, so make something up
...
/////////////////////////////////////////////////////////
// Given a plane and a point, return the point on the plane closest the point.
...
// ----------------------------------------------------------------------------
// This routine determines the resultant angular displacement and angular
// velocity in trying to reach a goal given an angular velocity APPROACHing
// a goal.  It uses maximal acceleration to a point (called peak), then maximal
// deceleration to arrive at the goal with zero angular velocity.  This can
// occasionally cause overshoot.
// w_in         > 0
// w_max        > 0
// theta_goal   > 0
// aa           > 0
// returns delta_theta
static float away(float w_in, float w_max, float theta_goal, float aa,
				  float delta_t, float *w_out, int no_overshoot);
static float approach(float w_in, float w_max, float theta_goal, float aa,
					  float delta_t, float *w_out, int no_overshoot)
{
	...
	if (w_in * w_in > 2.0f * 1.05f * aa * theta_goal) {
		// overshoot condition
		effective_aa = 1.05f * aa;
		delta_theta = w_in * delta_t - 0.5f * effective_aa * delta_t * delta_t;

		if (delta_theta > theta_goal) {
			// pass goal during this frame
			float t_goal = (-w_in +
							fl_sqrt(w_in * w_in +
									2.0f * effective_aa * theta_goal)) / effective_aa;
			...
			delta_theta -= away(-w_in, w_max, 0.0f, aa,
								delta_t - t_goal, w_out, no_overshoot);
			*w_out = -*w_out;
	...
	} else if (w_in * w_in < 2.0f * 0.95f * aa * theta_goal) {
...
// This routine determines the resultant angular displacement and angular
// velocity in trying to reach a goal given an angular velocity AWAY from
// a goal.  It uses maximal acceleration to a point (called peak), then maximal
...

{
	...
	if (t0 > delta_t) {
		// no reversal in this time interval
		*w_out = w_in + aa * delta_t;
	...
	t_excess = delta_t - t0;
	delta_theta += approach(0.0f, w_max, theta_goal, aa,
							t_excess, w_out, no_overshoot);
...
void vm_matrix_interpolate(const matrix *goal_orient, const matrix *curr_orient,
						   const vec3d *w_in, float delta_t,
						   matrix *next_orient, vec3d *w_out,
						   ...
						   int no_overshoot)
{
	...
	vec3d theta_goal;  // desired angular pos at the end of the time interval
	vec3d theta_end;   // actual angular pos at the end of the time interval
	...
	// determine angle and rotation axis from curr to goal
	...
	if (theta_goal.xyz.x > 0) {
		if (w_in->xyz.x >= 0) {
			delta_theta = approach(w_in->xyz.x, vel_limit->xyz.x,
								   theta_goal.xyz.x, acc_limit->xyz.x,
								   delta_t, &w_out->xyz.x, no_overshoot);
			theta_end.xyz.x = delta_theta;
		} else {
			// w_in->xyz.x < 0
			delta_theta = away(w_in->xyz.x, vel_limit->xyz.x,
							   theta_goal.xyz.x, acc_limit->xyz.x,
							   delta_t, &w_out->xyz.x, no_overshoot);
			theta_end.xyz.x = delta_theta;
		}
	...
	if (theta_goal.xyz.y > 0) {
		if (w_in->xyz.y >= 0) {
			delta_theta = approach(w_in->xyz.y, vel_limit->xyz.y,
								   theta_goal.xyz.y, acc_limit->xyz.y,
								   delta_t, &w_out->xyz.y, no_overshoot);
			theta_end.xyz.y = delta_theta;
		} else {
			// w_in->xyz.y < 0
			delta_theta = away(w_in->xyz.y, vel_limit->xyz.y,
							   theta_goal.xyz.y, acc_limit->xyz.y,
							   delta_t, &w_out->xyz.y, no_overshoot);
			theta_end.xyz.y = delta_theta;
		}
	...
	if (theta_goal.xyz.z > 0) {
		if (w_in->xyz.z >= 0) {
			delta_theta = approach(w_in->xyz.z, vel_limit->xyz.z,
								   theta_goal.xyz.z, acc_limit->xyz.z,
								   delta_t, &w_out->xyz.z, no_overshoot);
			theta_end.xyz.z = delta_theta;
		} else {
			// w_in->xyz.z < 0
			delta_theta = away(w_in->xyz.z, vel_limit->xyz.z,
							   theta_goal.xyz.z, acc_limit->xyz.z,
							   delta_t, &w_out->xyz.z, no_overshoot);
			theta_end.xyz.z = delta_theta;
		}
	...
	// arrived at goal?
	if ((theta_end.xyz.x == theta_goal.xyz.x) &&
		(theta_end.xyz.y == theta_goal.xyz.y) &&
...
void get_camera_limits(const matrix *start_camera, const matrix *end_camera,
					   float time, vec3d *acc_max, vec3d *w_max)
{
...
// goal delta_bank.  Rotational velocity on x/y is rotated with bank, giving
// smoother motion.
void vm_forward_interpolate(const vec3d *goal_f, const matrix *orient,
							const vec3d *w_in, float delta_t, float delta_bank,
							matrix *next_orient, vec3d *w_out,
							const vec3d *vel_limit, const vec3d *acc_limit,
							int no_overshoot)
{
	...
	if (t < SMALLER_NUM) {
		if (z_dotprod > 0.0f) {
			...
			// the forward vector is pointing exactly opposite of goal
			// arbitrarily choose the x axis to rotate around until t
	...
	// no rotation if delta_bank and w_in both 0 or
	// rotational acc in forward is 0
	no_bank = ((delta_bank == 0.0f) && (vel_limit->xyz.z == 0.0f) &&
			   (acc_limit->xyz.z == 0.0f));
...
//
//  input:  pnt      => array of world positions
...
void vm_find_bounding_sphere(const vec3d *pnts, int num_pnts, vec3d *center, float *radius)
{
	...
	for (i = 0; i < num_pnts; i++) {
		...
		if (old_to_p_sq > rad_sq) {
			...
			center->xyz.x = ((rad * center->xyz.x) +
							 (old_to_new * p->xyz.x)) / old_to_p;
			center->xyz.y = ((rad * center->xyz.y) +
							 (old_to_new * p->xyz.y)) / old_to_p;
			center->xyz.z = ((rad * center->xyz.z) +
							 (old_to_new * p->xyz.z)) / old_to_p;
...
//
// given a last orientation and current orientation, estimate the next
// orientation
//
...
{
	return is_valid_vec(&m->vec.fvec) &&
		   is_valid_vec(&m->vec.uvec) &&
		   is_valid_vec(&m->vec.rvec);
}

// interpolate between 2 vectors. t goes from 0.0 to 1.0. at
void vm_vec_interp_constant(vec3d *out, const vec3d *v0, const vec3d *v1,
							float t)
{
	vec3d cross;
	float total_ang;
...
// randomly perturb a vector around a given (normalized vector) or optional
// orientation matrix
						...
						const matrix *orient)
{
	...
	// axis 1
	vm_rot_point_around_line(&t1, in,
							 fl_radians(frand_range(-max_angle, max_angle)),
							 &vmd_zero_vector, &rot->vec.fvec);

	// axis 2
	vm_rot_point_around_line(&t2, &t1,
							 fl_radians(frand_range(-max_angle, max_angle)),
							 &vmd_zero_vector, &rot->vec.rvec);

	// axis 3
	vm_rot_point_around_line(out, &t2,
							 fl_radians(frand_range(-max_angle, max_angle)),
							 &vmd_zero_vector, &rot->vec.uvec);
}

void vm_vec_random_cone(vec3d *out, const vec3d *in, float min_angle,
						float max_angle, const matrix *orient)
{
	vec3d t1, t2;
	const matrix *rot;
	matrix m;
	...
	vm_rot_point_around_line(&t1, in, fl_radians(temp_ang),
							 &vmd_zero_vector, &rot->vec.fvec);
	...
	vm_rot_point_around_line(&t2, &t1, fl_radians(temp_ang),
							 &vmd_zero_vector, &rot->vec.rvec);
	...
	vm_rot_point_around_line(out, &t2, fl_radians(temp_ang),
							 &vmd_zero_vector, &rot->vec.uvec);
}


// given a start vector, an orientation and a radius, give a point on the plane
//  of the circle if on_edge is 1, the point is on the very edge of the circle
void vm_vec_random_in_circle(vec3d *out, const vec3d *in, const matrix *orient,
							 float radius, int on_edge)
{
	vec3d temp;

	// point somewhere in the plane
	vm_vec_scale_add(&temp, in, &orient->vec.rvec,
					 on_edge ? radius : frand_range(0.0f, radius));

	// rotate to a random point on the circle
	vm_rot_point_around_line(out, &temp, fl_radians(frand_range(0.0f, 359.0f)),
							 in, &orient->vec.fvec);
}

// given a start vector, an orientation, and a radius, give a point in a
// spherical volume if on_edge is 1, the point is on the very edge of the sphere
...
// returns 0 if the point is inside the line segment, -1 if "before" the line
// segment and 1 ir "after" the line segment
...
// Goober5000
// Finds the distance squared to a line.  Same as above, except it uses
// vm_vec_dist_squared, which is faster; and it doesn't check whether the
...
{
	matrix4 inv;  // create a temp matrix so we can avoid getting a
				  // determinant that is 0
	float det;
	int i, j;
	...
	det = (m->a2d[0][0] * inv.a2d[0][0]) +
		  (m->a2d[0][1] * inv.a2d[1][0]) +
		  (m->a2d[0][2] * inv.a2d[2][0]) +
