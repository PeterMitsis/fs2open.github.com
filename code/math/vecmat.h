...
// normalization. If it is detected, it generates a Warning() and returns
// the vector 1, 0, 0.
...
// Return value indicated where on the line *nearest_point lies.  Between
// 0.0f and 1.0f means it's in the line segment.  Positive means beyond *p1,
...
// Find the bounding sphere for a set of points (center and radius are output
// parameters)
...
// given a start vector, an orientation and a radius, give a point on the
// plane of the circle
...
// spherical volume if on_edge is 1, the point is on the very edge of the
// sphere
...
// vm_vec_dist_squared, which is faster;
